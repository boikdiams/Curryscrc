...
function Invoke-WindowsUpdate {
    param([bool]$IncludeDrivers = $false, [int]$TimeoutMinutes = 90)
...
try {
    ...
    if ($bat) {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) { Write-Host '         battery-report.html saved (check design vs full-charge capacity)' -ForegroundColor DarkGray }
        } catch { }
    } else {
    ...
    if ($SkipWindowsUpdate) {
        ...
        $wu = Invoke-WindowsUpdate -IncludeDrivers $UpdateIncludeDrivers -TimeoutMinutes $UpdateTimeoutMinutes
...
2. Hardware inventory.
3. Automatic checks: storage health, battery (+ battery-report.html), WiFi,
   Bluetooth, camera / audio / mic device presence, Windows licence / DPK.
4. Windows Update: search, download, install, report reboot status.
...
---------------------
   ...
   Finish-Repair.cmd -SkipWindowsUpdate
...
- Windows Update installs software updates only by default. Driver updates are
  off ($UpdateIncludeDrivers) because WU drivers are a common cause of
  "it worked when it left the bench" callbacks.
...
Write-Host 'End a job:    double-click Finish-Repair.cmd  (add -RunSfc for SFC)'
Write-Host ''
Read-Host 'Press Enter to close' | Out-Null
